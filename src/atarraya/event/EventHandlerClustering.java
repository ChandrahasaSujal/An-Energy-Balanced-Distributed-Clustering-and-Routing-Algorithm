 ...
 * @author Kitty
 */
public class EventHandlerClustering implements EventHandlerSW,constants {
    ...
    public EventHandlerClustering(atarraya_frame _father)
    {
    ...
    public void initial_event(int _id, int _treeID) {
                ...
                try {
                   ...
                   for(int j=0;j<father.getVariable(NUMPOINTS);j++)
                   {
                       if(j%2==0)
                       {
                           father.getNode(j).setEnergy(100);
                            father.getNode(j).startDeadTimer();
                       }
                       else if(j % 3 == 0)
                       {
                           father.getNode(j).setEnergy(200);
                           father.getNode(j).startDeadTimer();
                       }
                        else
                       {
    ...
    class ClusterProcessTimer extends TimerTask
    {
        ...
        public void run()
        {
            ...
            father.clearGreenLines();
            father.clearBlueLines();

            father.repaint();

            atarraya_frame.DEAD_NODE_MAP.put(rounds,atarraya_frame.DEAD_NODES);
            ...
            double weight=father.getNode(j).getEnergy()*(father.getNode(j).getEnergy()/neibours.length);
